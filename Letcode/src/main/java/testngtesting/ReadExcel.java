...
public static String[][] main() throws IOException {
	...
	int lastRowNum = sheet.getLastRowNum(); //to know no. of rows 
	System.out.println("no.of rows= "+lastRowNum);  // testng exclude header row (1st)by default
	int physicalNumberOfRows = sheet.getPhysicalNumberOfRows(); //this func include header row
	System.out.println("Includes header row= "+physicalNumberOfRows);
	short lastCellNum = sheet.getRow(0).getLastCellNum(); // have to find header to find last column bcz 
	                                                      //sometimes we type some reference in cell
...
//			String stringCellValue = cell.getStringCellValue();
	...
	for (int i =1; i <= lastRowNum; i++) {
		...
		data[i-1][j]= sheet.getRow(i).getCell(j).getStringCellValue();
  		...
  		//or data[i][j]= sheet.getRow(i+1).getCell(j).getStringCellValue(); //if we put for (int i =0; i<lastRowNum; i++) 
